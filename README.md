...
- [`docs/deep-links.md`](docs/deep-links.md) — **release:** real `chatapp://` links; **Expo Go only:** `exp://` + `bunx uri-scheme`
...
- **Navigation chrome** — Auth and stack settings routes use the **native stack** header (themed `ThemeProvider` + `createAppNavigationTheme()` in `src/utils/navigation-theme.ts`, functional `screenOptions` reading `theme`). **Inline titles** only (`headerLargeTitleEnabled: false` — large titles hid titles with short/centered `ScrollView`s). **Sign in** has a header title and **`headerBackVisible: false`** (root of logged-out stack). Other auth screens use the normal back control. The **signed-in** drawer + native tabs keep React Navigation **`headerShown: false`**; **Chat** and **Settings** each use an in-content **`TabScreenHeader`** (`src/components/tab-screen-header.tsx`) — centered **tab title** + **☰** to open the drawer.
- **Chat** (`src/app/(main)/(tabs)/index.tsx`):
  ...
  - **Drawer** — Logged-in shell uses `expo-router/drawer` around native tabs (`@react-navigation/drawer` + `react-native-drawer-layout`). **☰** on Chat or Settings opens the drawer; **New chat** clears the in-memory thread, aborts any active stream, navigates to the Chat tab, and uses a **pending** flag if the chat screen isn’t mounted yet (`src/ctx/chat-actions-context.tsx`).
